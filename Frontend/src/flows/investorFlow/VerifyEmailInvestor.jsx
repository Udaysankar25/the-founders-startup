  ...
  const handleVerify = async (e) => {
    ...
    try {
      const response = await fetch("/api/auth/verify-email", {
        ...
        body: JSON.stringify({ email, otp: enteredOtp, role: "investor" }),
      });
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-[1000px] bg-white rounded-[24px] shadow-lg flex flex-col lg:flex-row min-h-[540px]">
        {/* Left Panel */}
        <div
          className="relative w-full lg:w-[60%] bg-card px-6 md:px-10 lg:px-20 py-10 flex justify-center 
          ...
          {showPopup && (
            ...
            >
              <button
                ...
                className="absolute top-3 right-3 text-[#800080] text-lg font-bold"
              >
                ×
              </button>
        ...
        >
          <div className="bg-white/10 border border-white/30 rounded-[20px] p-4 md:p-6 text-center max-w-md w-full">
            ...
            <p className="text-sm md:text-base">
              Thanks for joining our investor network! We’ve sent a code to{" "}
              <strong>{email}</strong>. Please enter it here to verify your
              email.
            </p>
          </div>
        </div>
