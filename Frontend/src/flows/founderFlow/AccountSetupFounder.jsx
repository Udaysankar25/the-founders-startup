...
const validationSchema = Yup.object({
  ...
  privacy: Yup.string()
    .oneOf(["public", "private"], "Select privacy")
...
const AccountSetupFounder = () => {
  ...
  const onSubmit = async (values) => {
    ...
    setSubmitting(true);
    try {
      await apiCall("/api/auth/request-otp", "POST", {
        name: values.name,
        email: values.email,
      ...
      navigate("/founder/verify-email", { state: { email: values.email } });
    } catch (err) {
      setServerError(err.message || "Failed to start signup");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        >
          <div className="bg-white/10 border border-white/30 rounded-[24px] p-4 md:p-6 text-center max-w-md w-full">
            ...
            <p className="text-sm md:text-base">
              Start your journey—create your founder profile and begin building
              bold ideas.
            </p>
          </div>
        </div>
                ...
                <Form className="space-y-5">
                  ...
                  <div className="pt-4 flex justify-end">
                    <button
                      type="submit"
                      disabled={submitting}
                      className="flex items-center gap-2 px-6 py-2.5 bg-primary text-white rounded-full font-semibold hover:bg-primary/90 disabled:opacity-60"
                    >
