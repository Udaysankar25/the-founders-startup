...
export const googleSignup = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      if (user.authMethod === "google" && user.role === role) {
        // User already signed up via Google with this role
        return res.status(200).json({
          message: "User already signed up with Google",
          user,
          token: generateToken(user._id),
        });
      } else if (user.authMethod === "google" && user.role !== role) {
        return res
          .status(400)
          .json({ message: "Email already registered with a different role" });
      } else {
