...
export const requestOTP = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[Signup OTP] Sent to ${email} | OTP: ${otp} | Expires: ${new Date(
        otpExpires
      )}`
    );
...
export const completeSignup = async (req, res) => {
  ...
  try {
    ...
    const user = await User.create({
      ...
      github,
      website,
      role: session.role || role || "founder",
    });
...
export const resendOTP = async (req, res) => {
  ...
  if (!session)
    return res
      .status(404)
      .json({ message: "Session not found. Start signup again." });
  ...
  try {
    ...
    console.log(
      `[Resend OTP] Sent to ${email} | OTP: ${otp} | Expires: ${new Date(
        otpExpires
      )}`
    );
...
export const forgotPasswordRequest = async (req, res) => {
  ...
  if (lastSent && Date.now() - lastSent < 30 * 1000) {
    return res
      .status(429)
      .json({ message: "Wait 30 seconds before requesting another OTP" });
  }
  ...
  try {
    ...
    console.log(
      `[Forgot OTP] Sent to ${email} | OTP: ${otp} | Expires: ${new Date(
        otpExpires
      )}`
    );
...
export const resetPassword = async (req, res) => {
  ...
  try {
    ...
    if (!user || !user.resetPasswordVerified) {
      return res
        .status(400)
        .json({ message: "OTP not verified or request invalid" });
    }
